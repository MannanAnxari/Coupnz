...
const horizentalcoupon = ({ data, coupon, is_ico, img, isSingle }) => {
    ...
    return (
        <>
            {data?.Style === 1 ?
                <div className="bg-white my-2 h-100 p-3 coupon d-flex">
                    ...
                    <div className="col-8 my-auto px-2 px-md-4">
                        ...
                        {coupon.featured === 'on' ? <p class="new-tag m-0"><i class="fas fa-star  text-primary" aria-hidden="true"></i> New Coupon</p> : ''}
                    ...
                    <div className=" col-2 text-end m-auto my-auto ">
                        <Link href={`${coupon?.url || ''}`} onClick={() => { window.open(`/store/${coupon.store_slug}/${coupon?.id}`) }} className={`p-2 d-md-block fw-bold d-none button button-${!coupon?.code ? 'primary' : 'secondary'}`}  >
