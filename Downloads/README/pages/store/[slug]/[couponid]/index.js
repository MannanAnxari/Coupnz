...
const Storedetail = ({ data, metas, setMetas }) => {
  const dta = useRouter()
  let slug = dta?.query?.slug;
  let couponid = dta?.query?.couponid;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${APP_URL}api/single-store/${slug}?key=${APP_KEY}&cou=${couponid}`).then(res => res.json()).then(json => {
      setsinglestore(json);
      ...
      setMetas({ ...metas, title: `${data?.siteTitle ? data?.siteTitle : "Home"}`, metaTitle: data?.siteTitle ? data?.siteTitle : "", metaDescription: `${data?.meta ? data?.meta?.description : ""}`, metaKeyword: `${data?.meta ? data?.meta?.keywords : "More Coupon Codes"}` });
    })
