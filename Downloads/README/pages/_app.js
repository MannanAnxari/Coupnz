...
import { APP_URL, APP_KEY, CONTAINER_TYPE } from '@/public/settings/there_is_nothing_holding_me_back/config'
...
export default function App({ Component, pageProps }) {
    ...
    const [metas, setMetas] = useState({ title: data?.siteTitle ? data?.siteTitle : "Home", metaTitle: data?.siteTitle ? data?.siteTitle : "", metaDescription: `${data?.meta ? data?.meta?.description : ""}`, metaKeyword: `${data?.meta ? data?.meta?.keywords : "More Coupon Codes"}` });
    ...
    if (err) return <div
        ...
        return <div className={`_element ${CONTAINER_TYPE === 'wide' ? 'wide' : 'none-wide'}`}>
            <style jsx global>
                {
                    `:root{
            ...
            <Layout title={`${metas.title}`}
                metaTitle={`${metas.metaTitle}`}
                metaDescription={metas.metaDescription} logo=""
                metaKeywords={metas.metaKeyword}>

                {
                    data.Style === 1 ?
                        <Header1 data={data} category={category} season={season} coupons={coupons} country={country} /> :
                        <Header2 data={data} category={category} season={season} coupons={coupons} country={country} />
                }
                ...
                {
                    data.Style === 1 ?
                        <Footer1 data={data} category={category} season={season} coupons={coupons} country={country} /> :
                        <Footer2 data={data} category={category} season={season} coupons={coupons} country={country} />
                }
            </Layout>
